...
const initializeHierarchyRoutes = app => {
  ...
  hierarchyRouter.post("/", verifyAdmin, async (req, res) => {
    ...
    if (hierarchyDB !== 0) {
      return res
        .status(400)
        .send("Can not have more than one hierarchy in the DB");
    }
  ...
  hierarchyRouter.get("/", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send(
          `Could not get the hierarchy with id ${req.params.id} for ${e.message}`
        );
    }
  });
  ...
  hierarchyRouter.put("/", verifyAdmin, async (req, res) => {
    try {
      ...
      if (hierarchy === null || hierarchy.length === 0) {
        ...
        res.status(200);
        res.json("hierarchy updated successfully");
      }
    } catch (e) {
