...
const initializeFlashcardRoutes = (app) => {
    ...
    flashcardRouter.get('/', async (req, res) => {
        try {
            ...
            res.status(500).send(`Could not get the flashcards with query ${req.query} for ${e.message}`);
        }
    });
    ...
    flashcardRouter.delete('/:id', verifyAdmin,  async (req, res) => {
        ...
        try {
            ...
            res.status(500).send(`Could not remove the flashcard with id ${req.params.id} for ${e.message}`);
        }
    });
};
