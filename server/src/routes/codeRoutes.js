...
const initializeCodeRoutes = (app) => {
    ...
    codeRouter.post('/', verifyAdmin,  async (req, res) => {
        ...
        const adminCode = await CodeModel.findOne({createdBy: req.body.createdBy});
        if(adminCode){
            return res.status(400).send(`Admin ${adminCode.createdBy} already have a secret code`);
        }
        ...
        try {
            await code.save().then(() => res.send('Secret code has been created successfully. Please note it down in a safe place'));
        } catch (e) {
    ...
    codeRouter.post('/compare', async (req, res) => {
        ...
        if(!code) {
            return res.status(400).send(`We do not recognize ${req.body.authorizedBy}`);
        }
    ...
    codeRouter.delete('/', verifyAdmin,  async (req, res) => {
        ...
        try {
            const code = await CodeModel.findOneAndRemove({createdBy: req.query.createdBy});
            if (code === null) {
                return res.status(404).send(`No code found for admin ${req.query.createdBy}`);
            } else {
                res.status(200).send(`removed the secret code for admin ${req.query.createdBy}`);
            }
        } catch (e) {
