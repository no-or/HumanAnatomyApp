...
const initializeVideoRoutes = (app) => {
    ...
    videoRouter.get('/:id', async (req, res) => {
        try {
            ...
            if (video === null) {
                ...
                res.status(200);
                await res.json(video);
            }
        } catch (e) {
            res.status(500).send(`Could not get the video with id ${req.params.id} for ${e.message}`);
        }
    });
    ...
    videoRouter.get('/', async (req, res) => {
        try {
            ...
            res.status(500).send(`Could not get the videos with query ${req.query} for ${e.message}`);
        }
    });
    ...
    videoRouter.delete('/:id', verifyAdmin,  async (req, res) => {
        ...
        try {
            ...
            res.status(500).send(`Could not remove the video with id ${req.params.id} for ${e.message}`);
        }
    });
    ...
    videoRouter.delete('/', verifyAdmin,  async (req, res) => {
        ...
        console.log(title);
        if(!title) {
            res.status(400).send('Please pass title of the video that you want to remove as a query param');
        }
        try {
