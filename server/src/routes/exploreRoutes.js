...
const initializeExploreRoutes = app => {
  ...
  exploreRouter.post("/", verifyAdmin, async (req, res) => {
    ...
    try {
      ...
      res
        .status(400)
        .send(`Could not create the explorelab component for ${e.message}`);
    }
  });
  ...
  exploreRouter.get("/", async (req, res) => {
    try {
      ...
      if (data === null || data.length === 0) {
        res
          .status(404)
          .send(`No such exploreLab component found with query ${req.query}`);
      } else {
        res.status(200);
        res.json(data);
      }
      ...
      res
        .status(500)
        .send(
          `Could not get the exploreLab component with query ${req.query} for ${e.message}`
        );
    }
  });
  ...
  exploreRouter.delete("/:id", verifyAdmin, async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .send(
          `Could not remove the exploreLab component with id ${id} for ${e.message}`
        );
    }
  });
};
